# blaze/config/settings_dialog.py - Preferences dialog
...
from blaze.config.pascal_detector import (
    detect_pascal_compilers, is_valid_compiler,
)
...
class SettingsDialog(QDialog):
    ...
    def _create_build_tab(self):
        ...
        # Compiler group
        compiler_group = QGroupBox("Pascal Compiler")
        ...
        refresh_btn.setToolTip("Re-scan for installed Pascal compilers")
        ...
        self.compiler_path_edit.setPlaceholderText("Path to Pascal compiler executable")
        ...
        self.compiler_flags_edit.setPlaceholderText("-O2 -gl")
    ...
    def _refresh_compilers(self):
        ...
        self._detected_compilers = detect_pascal_compilers()
    ...
    def _browse_compiler(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Pascal Compiler Executable"
        )
        if path:
            self.compiler_path_edit.setText(path)
            self.custom_path_check.setChecked(True)
    ...
    def _load_values(self):
        ...
        self.tab_width_spin.setValue(
            self.settings.get("editor", "tab_width") or 2
        )
