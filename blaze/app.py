...
from blaze.browser.file_browser import FileBrowser
from blaze.build.build_manager import BuildManager
from blaze.config.settings import Settings
from blaze.config.settings_dialog import SettingsDialog
from blaze.config.themes import apply_theme_to_app, get_theme
from blaze.editor.find_replace import FindReplaceDialog
from blaze.editor.tab_widget import EditorTabWidget
from blaze.terminal.terminal_widget import TerminalWidget

_OPEN_FILTER = (
    "Pascal Files (*.pas *.pp *.p *.lpr *.lpk *.inc *.dpr *.dpk);;"
    "All Files (*)"
)
_SAVE_FILTER = (
    "Pascal Source (*.pas);;"
...
class MainWindow(QMainWindow):
    """BLAZE main application window."""

    def __init__(self):
        super().__init__()
        self.settings = Settings()
        self.setWindowTitle("BLAZE — Pascal Teaching Environment")
    ...
    def _create_menus(self) -> None:
        ...
        about_action = QAction("&About BLAZE", self)
    ...
    def open_file(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Pascal File", "", _OPEN_FILTER
        )
        if not path:
            return
        try:
    ...
    def save_file_as(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Pascal File", "", _SAVE_FILTER
        )
        if path:
    ...
    def show_about(self) -> None:
        QMessageBox.about(
            self,
            "About BLAZE",
            "BLAZE — BLAZing-fast Algorithmic Zone Environment\n"
            "Version 0.1.0\n\n"
            "A beginner-friendly IDE for learning and teaching Pascal.\n\n"
