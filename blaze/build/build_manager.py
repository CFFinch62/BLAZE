# blaze/build/build_manager.py - Compile + Run workflow manager for Pascal
...
from blaze.config.settings import Settings


class BuildManager(QObject):
    """Orchestrates the Save → Compile → Run workflow for Pascal sources.

    Uses the Free Pascal Compiler (fpc) or compatible (gpc, ppcx64, …).

    FPC command syntax:
        fpc [flags] -o<output_path> <source_path>

    Note: FPC uses ``-o<path>`` with no space between flag and path.
    ...
    def clean(self, source_path: str) -> None:
        """Delete the binary and FPC intermediate files for *source_path*.

        FPC produces three artefacts per compilation:
          - the binary (no extension)
          - <stem>.o  (object file)
          - <stem>.ppu  (Pascal unit file, if compiled as a UNIT)
        """
        output_path = self._get_output_path(source_path)
        ...
        # .o object file (FPC always produces this next to the source)
        obj = source_dir / f"{stem}.o"
        if obj.exists():
            obj.unlink()
            removed.append(str(obj))

        # .ppu unit file
        ppu = source_dir / f"{stem}.ppu"
        if ppu.exists():
            ppu.unlink()
            removed.append(str(ppu))

        if self.terminal:
            if removed:
                files_list = ", ".join(Path(r).name for r in removed)
                msg = (
                    f"\r\n\033[1;33m[BLAZE] Cleaned: {files_list}\033[0m\r\n"
                )
            else:
                msg = "\r\n\033[1;33m[BLAZE] Nothing to clean.\033[0m\r\n"
    ...
    def _build_command(self, source_path: str):
        ...
        FPC syntax:  fpc [flags] -o<path> <source>   (no space after -o)
        """
        compiler = self.settings.get("build", "compiler_path") or "fpc"
        flags_str = self.settings.get("build", "compiler_flags") or ""
        flags = flags_str.split() if flags_str.strip() else []
        output_path = self._get_output_path(source_path)
        # FPC requires -o immediately followed by the path (no space)
        args = flags + [f"-o{output_path}", source_path]
    ...
    def _start_compile(self, source_path: str) -> None:
        ...
        if self.terminal:
            header = (
                f"\r\n\033[1;34m[BLAZE] Compiling...\033[0m\r\n"
                f"$ {cmd_str}\r\n"
            )
        ...
        if exit_code == 0:
            ...
            if self.terminal:
                self.terminal.on_data_received(
                    b"\r\n\033[1;32m[BLAZE] Build successful.\033[0m\r\n"
                )
            ...
            if self.terminal:
                self.terminal.on_data_received(
                    f"\r\n\033[1;31m[BLAZE] Build failed (exit {exit_code}).\033[0m\r\n"
                    .encode()
                )
            self._pending_run = None
